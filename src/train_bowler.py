...
MODEL = MODELS / "bowler_model.pkl"

MODELS.mkdir(parents=True, exist_ok=True)

# Loading Data -----------------------------------
def load():
    df = pd.read_csv(BOWLER)
...
def prepare(df):
    feet = [
        "bowler", 'team', 'opp', 'venue', 'avg_wkts', 'avg_eco', 'avg_runs_given',
        'avg_venue_wkts', 'avg_opp_wkts'
    ]
...
def train_eval(x, y):
    ...
    print("\n>>> Bowler Wicket Model Prformance")
    print("-"*35)
    ...
    result = pd.DataFrame({
        ...
        "Predicted Wickets" : y_pred
    })
    result["Error"] = result["Actual Wickets"] - result["Predicted Wickets"]
